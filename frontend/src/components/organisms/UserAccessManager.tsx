    ...
    <div className="space-y-6">
      <Card className="border-2 border-gray-700/60 bg-gray-900/80 backdrop-blur-md shadow-xl">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex items-center justify-between rounded-lg border border-gray-700/60 bg-gray-800/40 px-4 py-3">
            ...
            <Switch
              checked={hasAllPermissions(inviteForm.permissions)}
              onCheckedChange={(checked) =>
                setInviteForm((current) => ({
                  ...current,
                  permissions: checked ? fullPermissions : emptyPermissions,
                  serverAccess: checked ? [] : current.serverAccess,
                }))
              }
            />
          </div>
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    setInviteForm((current) => ({
                      ...
                      serverAccess:
                        permission.key === 'accessAllServers' && checked ? [] : current.serverAccess,
                    }))
                  }
                />
              </div>
                ...
                {serverIds.map((serverId) => (
                  <button
                    ...
                    onClick={() =>
                      setInviteForm((current) => ({
                        ...current,
                        serverAccess: toggleServer(current.serverAccess, serverId),
                      }))
                    }
                    className={`rounded-lg border px-3 py-2 text-left text-sm transition-colors ${
                      inviteForm.serverAccess.includes(serverId)
          ...
          {latestInviteUrl ? (
            <div className="space-y-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-4">
              <Label className="text-emerald-200">{t('latestInvitationLink')}</Label>
              <Input readOnly value={latestInviteUrl} className="bg-gray-900 border-gray-700 text-emerald-200" />
            </div>
          ) : null}
          ...
          <div className="space-y-2">
            ...
            {invitations.map((invitation) => (
              <div key={invitation.id} className="rounded-lg border border-gray-700/60 bg-gray-800/40 px-4 py-3 text-sm text-gray-300">
      ...
      <Card className="border-2 border-gray-700/60 bg-gray-900/80 backdrop-blur-md shadow-xl">
        ...
        <CardContent className="space-y-4">
          {users.map((user) => (
            <div key={user.id} className="space-y-4 rounded-xl border border-gray-700/60 bg-gray-800/40 p-4">
                ...
                {user.role === 'USER' ? (
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="rounded-xl border border-red-500/40 bg-linear-to-b from-red-500 to-red-700 font-minecraft text-white shadow-lg shadow-red-950/40 transition-all hover:scale-[1.02] hover:from-red-400 hover:to-red-600 hover:shadow-red-900/50 active:scale-[0.98] disabled:scale-100 disabled:opacity-60"
                    >
              ...
              {user.role === 'ADMIN' ? (
                ...
                <>
                  <div className="flex items-center justify-between rounded-lg border border-gray-700/60 bg-gray-900/40 px-4 py-3">
                    ...
                    <Switch
                      checked={hasAllPermissions(user.access.permissions)}
                      onCheckedChange={(checked) =>
                        updateLocalUser(user.id, (current) => ({
                          ...
                          access: {
                            ...current.access,
                            permissions: checked ? fullPermissions : emptyPermissions,
                            serverAccess: checked ? [] : current.access.serverAccess,
                          },
                        }))
                      }
                    />
                  ...
                  <div className="flex items-center justify-between rounded-lg border border-gray-700/60 bg-gray-900/40 px-4 py-3">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => updateLocalUser(user.id, (current) => ({ ...current, isActive: checked }))}
                    />
                  </div>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {permissionLabels.map((permission) => (
                      <div key={permission.key} className="flex items-center justify-between rounded-lg border border-gray-700/60 bg-gray-900/40 px-4 py-3">
                        <span className="text-sm text-gray-200">{t(permission.label as never)}</span>
                        <Switch
                          checked={user.access.permissions[permission.key]}
                          onCheckedChange={(checked) =>
                            updateLocalUser(user.id, (current) => ({
                              ...current,
                              access: {
                                ...
                                serverAccess:
                                  permission.key === 'accessAllServers' && checked ? [] : current.access.serverAccess,
                              },
                            }))
                          }
                        />
                          ...
                          <button
                            ...
                            onClick={() =>
                              updateLocalUser(user.id, (current) => ({
                                ...
                                access: {
                                  ...current.access,
                                  serverAccess: toggleServer(current.access.serverAccess, serverId),
                                },
                              }))
                            }
                            className={`rounded-lg border px-3 py-2 text-left text-sm transition-colors ${
                              user.access.serverAccess.includes(serverId)
                                ? 'border-emerald-500 bg-emerald-500/10 text-emerald-300'
